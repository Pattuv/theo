...
1. **weather** — parameters: one string `location`. Use the user's place if they named one; otherwise use `"local"`. If the location contains a comma, keep it inside quotes. **Any question about current weather, today's weather, or local conditions without a named city must include** `_("weather","local")` — do not treat these as plain chat.
...
- User: what's the weather? / what's the weather today? / how's the weather? / is it raining?  
  **BAD:** `---CHAT---` — no tool segment; user asked for weather.  
  **GOOD:** `---CHAT---_("weather","local")` — no city named, so use `"local"`.
...
4. Base classification and tool use on user **intent**, not exact wording. When in doubt, **omit** tool segments — **except weather**: if the user wants current or local weather and did not name a place, always append `_("weather","local")`.
